...
class ImageStudioTests(unittest.TestCase):
    def setUp(self):
        ...
        path = self.project/"MGPU.srcs/sim_1/new/top_tb.v"
        path.parent.mkdir(parents=True)
        path.write_text((Path(__file__).resolve().parents[1]/"MGPU.srcs/sim_1/new/top_tb.v").read_text(),encoding="utf-8")
    ...
    def test_generate_writes_testbench_and_invalidates_after_moving(self):
        ...
        self.assertTrue((self.project/"out/top_image_tb.v").exists())
        self.assertEqual(str(studio.export_button.cget("state")),"normal")
        ...
        self.assertEqual(str(studio.export_button.cget("state")),"disabled")
        self.assertFalse(self.errors)
