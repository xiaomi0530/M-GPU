...
def save_bundle(mesh: Mesh, project: Path, source: str = "") -> dict[str, Path]:
    ...
    paths = {"scene": output / "image_scene.tri", "testbench": output / "mgpu_image_tb.v",
    ...
    template = (project / "MGPU.srcs/sim_1/new/mgpu_tb.v").read_text(encoding="utf-8")
    marker = 'localparam DEFAULT_IMAGE_SCENE = "out/image_scene.tri";'
    if marker not in template or "module mgpu_tb;" not in template:
        raise ValueError("mgpu_tb.v 缺少图片场景入口，请使用项目中的新版 testbench。")
    scene_literal = str(paths["scene"]).replace("\\", "/").replace('"', '\\"')
    testbench = template.replace("module mgpu_tb;", "module mgpu_image_tb;", 1).replace(
        marker, f'localparam DEFAULT_IMAGE_SCENE = "{scene_literal}";', 1)
    testbench = "// Generated by Image Studio. Compile with -s mgpu_image_tb.\n" + testbench
    scene = str(len(mesh.triangles)) + "\n"
    scene += "".join(" ".join(map(str, tri.values[:6])) + " " +
                     " ".join(f"{v:02x}" for v in tri.values[6:]) + "\n" for tri in mesh.triangles)
    metadata = {"source": source, "placement": vars(mesh.placement), "triangles": len(mesh.triangles),
